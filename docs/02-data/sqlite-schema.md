...
Definir DDL e indices para estado, auditoria, idempotencia y reproceso controlado.
...
- `updates.update_id` como `PRIMARY KEY`.
- `publications.update_id` como `PRIMARY KEY` (una publicacion logical por update).
- En reintentos:
  - si `updates` ya existe, no repetir pipeline completo.
  - consultar `publications.status` para decidir compensacion/reproceso parcial.

## Query de reproceso manual

```sql
SELECT u.update_id, p.status, p.last_error, MAX(e.created_at) AS last_event_at
FROM updates u
LEFT JOIN publications p ON p.update_id = u.update_id
LEFT JOIN events e ON e.update_id = u.update_id
WHERE p.status IN ('failed','partial')
GROUP BY u.update_id, p.status, p.last_error
ORDER BY last_event_at DESC;
```

## Migraciones

- Usar `alembic` para evolucionar schema sin editar DDL historico.
- Cada migracion debe tener rollback documentado.
